...
prequel_box <- c(phantom_menace,  attack_of_clones, revenge_of_sith)
# create matrix
prequel_trilogy_matrix <- matrix(prequel_box, byrow=T, nrow=3)
#calculate total revenue
total_prequel_revenue <- rowSums(prequel_trilogy_matrix)
# label the rows of prequel_trilogy_matrix
rownames(total_prequel_matrix)[1,3] <- c("phantom_menace","attack_of_clones","revenge_of_sith")
# append total_prequel_revenue to Prequel_trilogy_matrix
prequel_trilogy_matrix <- cbind(prequel_trilogy_matrix, total_prequel_revenue)
#label the last column of prequel_trilogy_matrix as "Total revenue"
colnames(total_prequel_matrix)[3] <- "Total revenue"
# bind star_wars_matrix and prequel_trilogy_matrix using rbind
wars_prequel_matrix <- rbind(star_wars_matrix, prequel_trilogy_matrix)

# Matrices are understood by Rtotal_prequel_revenue-dimensional, because they are vectors folded onto themselves
...
binomial_distribute <- function(n, k, p){
  return ( choose(n, k) * p^k * (1-p)^(n-k) )
}
# tial: binomial_distribute(10,3,3/5) returns  0.04246733
