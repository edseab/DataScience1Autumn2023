...
star_wars_matrix["Phantom menace", "International revenue"]
...
comp.result <- c(new_hope[1]*phantom_menace[1], empire_strikes[1]*attack_of_clones[1], return_jedi[1]*revenge_of_sith[1])
...
my_t <- function(x1,x2){
  ...
  df <- (s^2)^2/((s1^2/n1)^2/(n1-1)) + (s2^2/n2)/(n2-1)
...
Lecture_case_study
x <- (c(76,120,87,92,138,180,119,110,35,101))
sd(x)
pt(1-P)
pt(0.475,9, lower.tail=F)
...
p1 <- pt(abs(t), df=df,lower.tail=F)
  ...
  My height
  pnorm(169,mean(men),sd(men))
  Yours
   pnorm(173,mean(men),sd(men))
