...
dataVec <- c(phantom_menace, attack_of_clones, revenge_of_sith)
prequel_trilogy <- matrix(data = dataVec, nrow = 3,  byrow = T)
prequel_trilogy <- cbind(prequel_trilogy, rowSums(prequel_trilogy))
rownames(prequel_trilogy) <- c("Phantom Menace", "Attack of Clones", "Revenge of Sith") 
