...
# and append them to star_wars_matrix using the function rbind (row bind)
 ...
 prequel_matrix<- matrix(c(phantom_menace,
                            attack_of_clones,
                            revenge_of_sith), byrow = T, nrow = 3)

prequel_matrix <- cbind(prequel_matrix, rowSums(prequel_matrix))

row.names(prequel_matrix) <- c("The phantom Menance","Attack of the Colnes", "Revenge of the Sith")
...
star_wars_matrix["The phantom Menance", "Total revenue"]
