...
preq_trilogy<-matrix(c(phantom_menace,attack_of_clones,revenge_of_sith),byrow=T,nrow=3)
preq_trilogy<-cbind(preq_trilogy,rowSums(preq_trilogy))
preq_trilogy
row.names(preq_trilogy)<-c("phantom_menace","attack_of_clones","revenge_of_sith")
preq_trilogy
star_wars_matrix<-rbind(star_wars_matrix,preq_trilogy)
star_wars_matrix
...
proba_func<-function(n,k,p){
  pr = p^k*(1-p)^((n-k))*(factorial(n)/(factorial(k)*factorial(n-k)))
  return(pr)
  
}
x<-proba_func(3,1,0.7)   #testing quiz 2.2
x
...
prob_alien_earth<-proba_func(10,8,0.7)
prob_alien_earth
# compare this to dbinom(8,10,0.7)
dbinom(8,10,0.7)  # it's the same :)
