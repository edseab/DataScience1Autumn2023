...
# Often, functions take multiple inputs with different purpose. Note how:
rep(3,4)
...
#f2 tells us whether one number is the multiple of another.
...
cast.2.dice <- function(x){
  one.die <- cast.die(x)
  second.die <- cast.die(x)
  two.dice <- one.die + second.die
  print(one.die)
  print(second.die)
  return(two.dice)
}
...
set.seed(105) #to get the same answer as the instructor
