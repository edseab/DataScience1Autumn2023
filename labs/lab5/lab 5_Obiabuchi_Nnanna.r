...
view(summary_table)
#rm()- to remove from the global enviromnet
#rm(list=ls())- to remove everything from the global enviromnt
view(mtcars)
...
mutate(Petal.Area= Petal.Length*Petal.Width)
 %>%
 group_by(Species) %>%
 summarise(mn_sepal.length=mean(Sepal.Length),
 sd_sepal.length= sd(Sepal.Length),
 mn_sepal_width= mean(Sepal.Width),
 sd_sepal_width= sd(Sepal.Width),
 mn_petal.area= mean(Petal.Area),
...
ggplot(mpg, aes(displ, hwy, color= )) + geom_point()
