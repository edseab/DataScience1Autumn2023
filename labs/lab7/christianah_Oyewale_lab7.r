...
#The estimate for the intercept is 7.4 mm. This means that the predicted tooth length of a Guinea pig that was NOT given any VC is 7.4.
...
#Or we say, in 96% of samples we would expect the true value of the intercept and slope to fall within our confidence intervals.
...
predict(tooth_growth_vc, new_data, interval='confidence', level=0.95)
...
new_data <- data.frame(dose=seq(0,2,0.1))
predictions <- as.data.frame(predict(tooth_growth_vc, new_data, interval='confidence', level=0.95))
...
#if you are only interest in drawing the line without the CI
